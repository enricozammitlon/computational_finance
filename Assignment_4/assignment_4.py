...
with open('data/varying_jmax.csv', newline='\n') as csvfile:
    reader = csv.DictReader(csvfile,fieldnames=['sMax','iMax','jMax','S','V'],quoting=csv.QUOTE_NONNUMERIC)
...
plt.figure()
plt.plot(variationData[1]['x'],variationData[1]['y'],label=r'$\beta=0.486,\sigma=3.03,iMax=%i,sMax=%i$'%(variationData[1]['iMax'],variationData[1]['sMax']),linewidth=2)
plt.xlabel('jMax')
plt.ylabel(r'$V(S=X,t=0)$')
plt.legend(loc='upper center',fancybox=False, framealpha=0.0)
plt.grid()
plt.savefig('images/european_varying_jmax.png',bbox_inches='tight', pad_inches=0.2)
...
ax1.grid()
ax1.scatter(np.asarray(variationData[2]['x'][:20]),variationData[2]['y'][:20],label=r'$V(S=X,t=0)$ for $\beta=0.486,\sigma=3.03,iMax=%i$'%(variationData[2]['iMax']))
ax2 = ax1.twinx()
ax2.set_ylabel(r'$jMax$')
fig.tight_layout()
ax2.plot(np.asarray(variationData[2]['x'][:20]),variationData[2]['jMax'][:20],label=r'$jMax$',color="orange")
...
ax1.set_ylabel(r'$V(S=X,t=0)$')
ax1.grid()
ax1.scatter(np.asarray(variationData[2]['x']),variationData[2]['y'],label=r'$V(S=X,t=0)$ for $\beta=0.486,\sigma=3.03,iMax=%i$'%(variationData[2]['iMax']))
ax2 = ax1.twinx()
ax2.set_ylabel(r'$jMax$')
fig.tight_layout()
ax2.plot(np.asarray(variationData[2]['x']),variationData[2]['jMax'],label=r'$jMax$',color="orange")
lines, labels = ax1.get_legend_handles_labels()
lines2, labels2 = ax2.get_legend_handles_labels()
ax2.legend(lines + lines2, labels + labels2,loc='lower right',fancybox=False, framealpha=0.0)
